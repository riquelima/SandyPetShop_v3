...
async def run_test():
    ...
    try:
        ...
        # -> Fill in pet and owner information including condominium address and proceed to service selection.
        ...
        frame = context.pages[-1]
        # Input owner address with condominium details
        elem = frame.locator('xpath=html/body/div/div/main/form/div/div[4]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Condominium ABC, Apt 101')
        ...
        frame = context.pages[-1]
        # Click Next to proceed to service selection
        elem = frame.locator('xpath=html/body/div/div/main/form/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select 'Pet Móvel' service and proceed to next step for address input and scheduling.
        frame = context.pages[-1]
        # Select 'Pet Móvel' (mobile grooming) service
        elem = frame.locator('xpath=html/body/div/div/main/form/div/div/div/button[4]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select a condominium (e.g., Vitta Parque) and click 'Próximo →' to proceed.
        frame = context.pages[-1]
        # Select condominium 'Vitta Parque'
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Appointment Successfully Scheduled at Condominium Vitta Parque').first).to_be_visible(timeout=3000)
        except AssertionError:
            raise AssertionError("Test failed: Mobile grooming appointment scheduling failed to confirm the correct condominium address and validate address input as per the test plan.")
