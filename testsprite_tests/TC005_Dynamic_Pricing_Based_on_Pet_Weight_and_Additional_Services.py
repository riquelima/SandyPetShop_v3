...
async def run_test():
    ...
    try:
        ...
        # -> Input owner address and WhatsApp, then click Próximo to go to services step.
        ...
        frame = context.pages[-1]
        # Click Próximo to go to the next step (Serviços)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Preço atualizado com sucesso').first).to_be_visible(timeout=3000)
        except AssertionError:
            raise AssertionError('Test case failed: The displayed price did not update in real-time when pet weight or additional services were changed during scheduling, as required by the test plan.')
